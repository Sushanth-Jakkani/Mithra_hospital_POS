...
-- Description: Ensure logo_url column exists on organizations table, create public storage bucket 'logos', and set up RLS policies

-- 1. Ensure logo_url column exists on organizations table
ALTER TABLE IF EXISTS organizations ADD COLUMN IF NOT EXISTS logo_url TEXT;

-- 2. Create the storage bucket if it does not exist
